...
- TUI의 `r`은 캐시 우선 조회, `R`은 캐시와 백오프를 건너뛴 HTTP 직접 조회입니다.
  `R`이 실패하면 직전 데이터와 함께 실제 오류 문구를 표시합니다.
...
그래서 낡았을 때 직접 조회 말고는 방법이 없습니다. 직접 조회에 성공하면 agentmeter는
...
`User-Agent`와는 무관합니다. `agentmeter/<version>`과 `claude-cli/2.1.234`로 각각
보내 봤지만 둘 다 429 였습니다.
