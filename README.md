...
마지막 줄은 그 숫자가 **언제 기준인지** 밝힙니다. Claude provider는 로컬 캐시를 읽을 수 있어서
...
agentmeter web --agent claude   # 설정과 관계없이 Claude Code만 표시
...
`web`은 브라우저용 서버와 터미널 상주 화면을 **함께** 띄웁니다. 화면 상단에 서버가
돌고 있다는 사실과 접속 주소가 붙어 있어서, 주소를 확인하려고 스크롤을 되짚을 필요가
없습니다.
...
터미널 화면은 `--watch`와 같습니다 — 같은 게이지에 같은 키(`r`·`R`·`q`)를 쓰고,
`q`로 나가면 웹 서버도 함께 내려갑니다. 두 화면은 하나의 조회 세션을 공유하므로
provider 조회는 화면 수와 무관하게 한 번만 나갑니다. 브라우저에서 새로고침을 누르면
터미널 화면도 같은 값으로 갱신됩니다.

출력이 터미널이 아니면(파이프, 백그라운드 실행) 화면 없이 주소만 출력하고 서버만
남습니다 — `agentmeter web: http://127.0.0.1:54321`.

기본적으로 `127.0.0.1`과 운영체제가 고른 ephemeral port를 사용합니다. `--port`로 포트를,
...
자세한 내용은 [Claude 조회 정책](docs/claude-provider.md)을 보세요.
...
Claude provider가 캐시를 먼저 읽는 것도 이 때문입니다 — 기본 경로에서는 네트워크를 쓰지 않습니다.
...
라이브러리의 지원 interface는 `run_agentmeter`입니다.
...
작업 트리에서 설치한 개발 빌드는 버전에 커밋 해시가 붙습니다 — `2026.8.2-2879224`,
커밋하지 않은 변경이 있으면 `2026.8.2-2879224-dirty`. 릴리즈 태그를 그대로 체크아웃한
깨끗한 트리에서만 `2026.8.2`가 됩니다. 지금 돌고 있는 바이너리가 어떤 코드인지
`agentmeter --version`으로 확인할 수 있습니다.
